...
bool CGLTexture::createTexture()
{
	...
	glTexParameterf(target, GL_TEXTURE_MAG_FILTER, GL_LINEAR); // GL_LINEAR_MIPMAP_LINEAR
	glTexParameterf(target, GL_TEXTURE_MIN_FILTER, GL_LINEAR); // GL_LINEAR_MIPMAP_LINEAR
...
bool CGLTexture::loadPNG(const char *filename, bool updateOnly)
{
	...
	glTexImage2D(target, 0, GL_RGBA32F, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, data);

	/*int err = gluBuild2DMipmaps(target, 3, width, height, GL_RGB, GL_UNSIGNED_BYTE, data);
	if (err)
	{
