...
MotionInterpolationTask::
	MotionInterpolationTask(MotionInterpolationArgs args)
	: m_MovieClipPattern(args.clip), m_FrameStart(args.framestart),
	  m_LocalWorkSize(cl::NDRange(args.localSize, args.localSize)),
	  m_UseDiamond(args.useDiamond), m_MVecIters(args.mvecIters)
{
	for (int i = 0; i < 255; i++)
		m_KeyboardMask[i] = false;
}

MotionInterpolationTask::~MotionInterpolationTask()
{
	ReleaseResources();
}
...
bool MotionInterpolationTask::InitResources(cl::Device Device, cl::Context Context)
{
	string empty = applypattern(m_MovieClipPattern, m_FrameStart);
	string paths[] = {
		empty, applypattern(m_MovieClipPattern, m_FrameStart + 1)};
	int i = 0;
	...
	for (int i = 0; i < m_images.size(); i++)
	{
		...
		// img = clCreateBuffer(Context, CL_MEM_READ_WRITE, GL_TEXTURE_2D, nullptr, &clError);
		int flags = CL_MEM_READ_WRITE; //i == 2 ? CL_MEM_WRITE_ONLY : CL_MEM_READ_ONLY;
		image.clImage = cl::ImageGL(Context, flags, GL_TEXTURE_2D, 0, image.texture->getID(), &clError);
		image.clBuffer = cl::Buffer(Context, CL_MEM_READ_WRITE, sizeof(cl_float) * 4 * m_ImgResX * m_ImgResY);
	...
	m_kernelEstimateMotion = new cl::KernelFunctor<RR_KERNEL_ARG, int>(m_Program, "EstimateMotion");
	m_kernelShiftVectors = new cl::KernelFunctor<RR_KERNEL_ARG, float>(m_Program, "ShiftVectors");
	m_kernelRenderFrame = new cl::KernelFunctor<RR_KERNEL_ARG, float>(m_Program, "RenderFrame");
	m_kernelRenderFrameBothDirs = new cl::KernelFunctor<RR_KERNEL_ARG, float>(m_Program, "RenderFrameBothDirs");
	m_kernelNormalizeMVecs = new cl::KernelFunctor<RR_KERNEL_ARG>(m_Program, "NormalizeMVecs");
	return true;
}
...
void MotionInterpolationTask::LoadMarchImage(cl::Context Context)
{
	...
	if ((int)simulSecs == m_MarchOffset + m_MarchIdx)
		return;
...
void MotionInterpolationTask::ComputeGPU(cl::Context &Context, cl::CommandQueue &CommandQueue)
{

	//get global work size
	cl::NDRange global = cl::NDRange(CLUtil::GetGlobalWorkSize(m_ImgResX, m_LocalWorkSize[0]), CLUtil::GetGlobalWorkSize(m_ImgResY, m_LocalWorkSize[1]));
	...
	if (!once || m_FrameCounter == 0)
	{
		...
		for (auto &img : m_images)
			cl_imgs.push_back(img.clImage);

		V_RETURN_CL(CommandQueue.enqueueAcquireGLObjects(&cl_imgs), "acq");
		for (auto &img : m_images)
		{
			V_RETURN_CL(CommandQueue.enqueueCopyImageToBuffer(img.clImage, img.clBuffer, {0, 0, 0}, {m_ImgResX, m_ImgResY, 1}, 0), "img->buf");
		}
...
#define CARGS enqueue, m_ImgResX, m_ImgResY, img_1, img_2, diffImg, mVecs, mVecsShifted, rendered
		...
		}
		{
			...
			fn(enqueue, m_ImgResX, m_ImgResY, img_1, img_2, unused, mVecsShifted, mVecsRevShifted, rendered, theta, error);
			V_RETURN_CL(error, "RenderFrameBothDirs");
		}
		for (auto &img : m_images)
		{
			//printf("copying\n");
			V_RETURN_CL(CommandQueue.enqueueCopyBufferToImage(img.clBuffer, img.clImage, 0, {0, 0, 0}, {m_ImgResX, m_ImgResY, 1}), "buf->img");
		}
...
void MotionInterpolationTask::RenderImg(Triforce &img, float x, float y)
{
	...
	if (m_InspectCloth)
	{
		...
		if (img.clothModel)
			img.clothModel->DrawGL(GL_TRIANGLES);
...
void MotionInterpolationTask::OnKeyboard(int Key, int KeyAction)
{
	...
	if (KeyAction == GLFW_PRESS)
	{
		...
		if (Key == GLFW_KEY_P)
		{
			fprintf(stderr, "rot=%f,%f,%f, trans=%f,%f,%f\n", m_RotateX, m_RotateY, 0.f, m_TranslateX, m_TranslateY, m_TranslateZ);
		}
...
{
	//move camera
	if (m_KeyboardMask[GLFW_KEY_E])
		m_TranslateZ += 0.5f * ElapsedTime;
	if (m_KeyboardMask[GLFW_KEY_Q])
		m_TranslateZ -= 0.5f * ElapsedTime;
	if (m_KeyboardMask[GLFW_KEY_D])
		m_TranslateX -= 0.5f * ElapsedTime;
	if (m_KeyboardMask[GLFW_KEY_A])
		m_TranslateX += 0.5f * ElapsedTime;
	if (m_KeyboardMask[GLFW_KEY_S])
		m_TranslateY += 0.5f * ElapsedTime;
