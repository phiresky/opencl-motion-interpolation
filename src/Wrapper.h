...
class Wrapper {
 public:
  ...
  //! We overload this method to define OpenGL callbacks and to support CL-GL
  //! context sharing
  ...
  // NOTE: these have to be static, as GLFW does not support ptrs to member
  // functions
  static void OnKeyboardCallback(GLFWwindow *Window, int Key, int ScanCode,
                                 int Action, int Mods);

  static void OnMouseCallback(GLFWwindow *Window, int Button, int State,
                              int Mods);
  static void OnMouseMoveCallback(GLFWwindow *window, double X, double Y);

  static void OnWindowResizedCallback(GLFWwindow *window, int Width,
                                      int Height);
 ...
 protected:
  ...
  virtual void OnKeyboard(GLFWwindow *pWindow, int Key, int ScanCode,
                          int Action, int Mods);
 ...
 protected:
  ...
  // for the physical simulation we need to keep track of the time between
  // frames rendered
  int m_FrameTimer;
