...
int sad(local uchar block1[TILE_Y * 3][TILE_X * 3], local uchar block2[TILE_Y * 3][TILE_X * 3], int2 b1pos, int2 b2pos, int2 localpos)
{
    //if (b1pos.x < 0 || b1pos.y < 0 || b1pos.x > 2 * TILE_X || b1pos.y > 2 * TILE_Y)
    //    return INF;
    if (b2pos.x < 0 || b2pos.y < 0 || b2pos.x > 2 * TILE_X || b2pos.y > 2 * TILE_Y)
        return INF;
    local int sum;
    ...
    atomic_add(&sum, abs(block1[b1lpos.y][b1lpos.x] - block2[b2lpos.y][b2lpos.x]));
    barrier(CLK_LOCAL_MEM_FENCE);
    return sum;
}
...
// void plotLine(global write_only float4 *img, uint width, uint height, int x0, int y0, int x1, int y1, float4 color0, float4 color1)

// c++ https://rosettacode.org/wiki/Xiaolin_Wu%27s_line_algorithm#C.2B.2B
void plotLineWu(global write_only float4 *img, uint width, uint height, float x0, float y0, float x1, float y1, float4 color, float4 color2)
{
...
void plotLine(global write_only float4 *img, uint width, uint height, int x0, int y0, int x1, int y1, float4 color0, float4 color1)
{
    ...
    for (;;)
    {
        ...
        {
            int2 pos = (int2)(x0, y0);
            float m = y1 - y0 > x1 - x0 ? (((float)y0 - zy0) / (y1 - y0)) : (((float)x0 - zx0) / (x1 - x0));
...
#define SETIFBESTBIASED(_loc)                                                            \
    {                                                                                    \
        ...
        int bias = (int)(biasWeight * distance((float2)(tspos.x, tspos.y), biasCenter)); \
...
int2 diamondSearch(local uchar block1[TILE_Y * 3][TILE_X * 3], local uchar block2[TILE_Y * 3][TILE_X * 3], int2 center, int2 lp, float2 biasCenter, float biasWeight)
{
    int2 searchpos = center; // (int2)(biasCenter.x, biasCenter.y); // center; // (int2)(biasCenter.x, biasCenter.y);
    //if (searchpos.x < 0 || searchpos.y < 0 || searchpos.x >= 2 * TILE_X || searchpos.y >= 2 * TILE_Y)
    //    searchpos = center;
    int i = 0;
    bool isSdsp = false;
    {
    ...
    while (true)
    {
        ...
        SETIFBEST((int2)(0, 0));
        /*if (get_group_id(0) == 4 && get_group_id(1) == 6 && lp.x == 0 && lp.y == 0)
        {
...
int2 exhaustiveSearch(local uchar block1[TILE_Y * 3][TILE_X * 3], local uchar block2[TILE_Y * 3][TILE_X * 3], int2 center, int2 lp, float2 biasCenter, float biasWeight)
{
...
kernel __attribute__((reqd_work_group_size(TILE_X, TILE_Y, 1))) void EstimateMotion(uint width, uint height,
                                                                                    global read_only float4 *img_1,
                                                                                    global read_only float4 *img_2,
                                                                                    global write_only float4 *diffImg,
                                                                                    global read_write float4 *mVecs,
                                                                                    global float4 *unused1,
                                                                                    global float4 *unused2,
                                                                                    int iteration)
{
...
#define MVIMAYB(delt)               \
    ...
    if (USE_DIAMOND)
    {
        searchpos = diamondSearch(block1, block2, center, lp, ((float2)(center.x + biasCenter.x, center.y + biasCenter.y)), biasWeight);
    }
    else
    {
        searchpos = exhaustiveSearch(block1, block2, center, lp, ((float2)(center.x + biasCenter.x, center.y + biasCenter.y)), biasWeight);
    }
...
#define WRITEDEBUG 0
    if (WRITEDEBUG)
    {
        ...
        float diff = (val1 + val2) / 512.f; //val1 / 256.f; //abs(val1 - val2) / 256.f;
        ...
        if (lp.x == TILE_X / 2 && lp.y == TILE_Y / 2)
        {
            ...
            if (OOB(ofsd))
                return;
            plotLineWu(diffImg, width, height, p.x, p.y, ofsd.x, ofsd.y, (float4)(0.5, 0.5, 0.5, 1), (float4)(1, 0, 0, 1));
...
kernel __attribute__((reqd_work_group_size(TILE_X, TILE_Y, 1))) void ShiftVectors(uint width, uint height,
                                                                                  global read_only float4 *img_1,
                                                                                  global read_only float4 *img_2,
                                                                                  global write_only float4 *diffImg,
...
kernel __attribute__((reqd_work_group_size(TILE_X, TILE_Y, 1))) void NormalizeMVecs(uint width, uint height,
                                                                                    global read_only float4 *img_1,
                                                                                    global read_only float4 *img_2,
                                                                                    global write_only float4 *diffImg,
                                                                                    global read_only float4 *mVecs,
                                                                                    ...
                                                                                    global float4 *_unused2)
{
    ...
    if (OOB(p))
        return;
    barrier(CLK_GLOBAL_MEM_FENCE);
    global read_write union hacky *mVecsShifted = (global read_write void *)_mVecsShifted;
...
kernel __attribute__((reqd_work_group_size(TILE_X, TILE_Y, 1))) void RenderFrame(uint width, uint height,
                                                                                 global read_only float4 *img_1,
                                                                                 global read_only float4 *img_2,
                                                                                 global write_only float4 *diffImg_unused,
                                                                                 global write_only float4 *mVecs_unused,
                                                                                 global read_only float4 *mVecsShifted,
                                                                                 global write_only float4 *rendered,
                                                                                 float t)
{
    int2 p = (int2)(get_global_id(0), get_global_id(1));
    if (OOB(p))
    ...
    int2 mv = (int2)(F2I(_mv.x), F2I(_mv.y));
    int2 mvScaled = (int2)((int)round(t * mv.x), (int)round(t * mv.y));
    int2 mvScaled2 = (int2)((int)round((1 - t) * mv.x), (int)round((1 - t) * mv.y));
...
kernel __attribute__((reqd_work_group_size(TILE_X, TILE_Y, 1))) void RenderFrameBothDirs(uint width, uint height,
                                                                                         global read_only float4 *img_1,
                                                                                         global read_only float4 *img_2,
                                                                                         ...
                                                                                         float t)
{
    ...
    if (_mvFwd.w == 0 && _mvBwd.w == 0)
    {
        ...
        idx(rendered, p) = mix(idx(img_1, p), idx(img_2, p), t); //(float4)(1, 0, 0, 1);
        return;
    }
    ...
    int2 mvFwdScaledBwd = (int2)((int)round((1 - t) * mvFwd.x), (int)round((1 - t) * mvFwd.y));

    int2 mvBwdScaledBwd = (int2)((int)round(t * mvBwd.x), (int)round(t * mvBwd.y));
    int2 mvBwdScaled = (int2)((int)round((1 - t) * mvBwd.x), (int)round((1 - t) * mvBwd.y));
