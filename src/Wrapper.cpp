...
Wrapper *Wrapper::GetSingleton() {
  if (!s_pSingletonInstance) s_pSingletonInstance = new Wrapper();
...
bool Wrapper::EnterMainLoop(int argc, char **argv) {
  if (argc < 6) {
    printf(
        "usage:   ./MotionInterpolation framepattern startframe blocksize "
        "use-diamond-pattern mveciters\n");
    printf(
        "example: ./MotionInterpolation \"Assets/ball-crop/ball%%04d.png\" 174 "
        "16 0 1\n");
    return false;
  }
...
bool Wrapper::InitCLContext() {
  ...
  V_RETURN_FALSE_CL(
      clGetPlatformIDs(c_MaxPlatforms, &platformIds[0], &countPlatforms),
  ...
  for (size_t i = 0; i < platformIds.size(); i++) {
    ...
    clGetDeviceIDs(platformIds[i], deviceType, 1, &deviceIds[countAllDevices],
                   &countDevices);
  ...
  if (countAllDevices == 0) {
    std::cout
        << "No device of the selected type with OpenCL support was found.";
    return false;
  }
  // Choosing the first available device.
  m_CLDevice = deviceIds[0];
  clGetDeviceInfo(m_CLDevice(), CL_DEVICE_PLATFORM, sizeof(cl_platform_id),
                  &m_CLPlatform, NULL);
  ...
  PRINT_INFO("Name", buffer, bufferSize, maxBufferSize,
             clGetPlatformInfo(m_CLPlatform, CL_PLATFORM_NAME, maxBufferSize,
                               (void *)buffer, &bufferSize));
  PRINT_INFO("Vendor", buffer, bufferSize, maxBufferSize,
             clGetPlatformInfo(m_CLPlatform, CL_PLATFORM_VENDOR, maxBufferSize,
                               (void *)buffer, &bufferSize));
  PRINT_INFO("Version", buffer, bufferSize, maxBufferSize,
             clGetPlatformInfo(m_CLPlatform, CL_PLATFORM_VERSION, maxBufferSize,
                               (void *)buffer, &bufferSize));
  PRINT_INFO("Profile", buffer, bufferSize, maxBufferSize,
             clGetPlatformInfo(m_CLPlatform, CL_PLATFORM_PROFILE, maxBufferSize,
                               (void *)buffer, &bufferSize));
  std::cout << std::endl << "Device:" << std::endl << std::endl;
  PRINT_INFO("Name", buffer, bufferSize, maxBufferSize,
             clGetDeviceInfo(m_CLDevice(), CL_DEVICE_NAME, maxBufferSize,
                             (void *)buffer, &bufferSize));
  PRINT_INFO("Vendor", buffer, bufferSize, maxBufferSize,
             clGetDeviceInfo(m_CLDevice(), CL_DEVICE_VENDOR, maxBufferSize,
                             (void *)buffer, &bufferSize));
  PRINT_INFO("Driver version", buffer, bufferSize, maxBufferSize,
             clGetDeviceInfo(m_CLDevice(), CL_DRIVER_VERSION, maxBufferSize,
                             (void *)buffer, &bufferSize));
  cl_ulong localMemorySize;
  clGetDeviceInfo(m_CLDevice(), CL_DEVICE_LOCAL_MEM_SIZE, sizeof(cl_ulong),
                  &localMemorySize, &bufferSize);
...
// Define OS-specific context properties and create the OpenCL context
  ...
  // Finally, create a command queue. All the asynchronous commands to the
  // device will be issued from the CPU into this queue. This way the host
  // program can continue the execution until some results from that device are
  // needed.

  m_CLCommandQueue = new cl::CommandQueue(m_CLContext, m_CLDevice,
                                          CL_QUEUE_PROFILING_ENABLE, &clError);
  V_RETURN_FALSE_CL(clError,
...
bool Wrapper::InitGL(int, char **) {
  ...
  m_Window = glfwCreateWindow(m_WindowWidth, m_WindowHeight,
                              "CL Visual Computing Demo", NULL, NULL);
  if (!m_Window) {
    glfwTerminate();
    exit(EXIT_FAILURE);
  }
  ...
  glLoadIdentity();
  gluPerspective(60.0, (GLfloat)m_WindowWidth / (GLfloat)m_WindowHeight, 0.1f,
                 10.0f);
...
void Wrapper::OnIdle() {
  if (m_PrevTime.time_since_epoch() == 0ns) {
    ...
    if (m_pCurrentTask) m_pCurrentTask->OnIdle(elapsedTime);

    DoCompute();
  }
}

void Wrapper::OnWindowResizedCallback(GLFWwindow *pWindow, int Width,
                                      int Height) {
...
void Wrapper::OnKeyboardCallback(GLFWwindow *pWindow, int Key, int ScanCode,
                                 int Action, int Mods) {
...
void Wrapper::OnMouseCallback(GLFWwindow *pWindow, int Button, int State,
                              int Mods) {
