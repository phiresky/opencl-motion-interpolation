...
CTriMesh::~CTriMesh() { ReleaseGLResources(); }
...
unsigned int CTriMesh::AddVertex(unsigned int Hash, Vertex *pVertex) {
  ...
  if (!found) {
    ...
    if (pCurrEntry == NULL) {
      ...
      // find the tail
      while (pCurrEntry->pNext != NULL) pCurrEntry = pCurrEntry->pNext;
...
CTriMesh *CTriMesh::CreatePlane(unsigned int nVertsX, unsigned int nVertsY,
  ...
  float deltaX = aspectRatio / xSegments;
  float deltaY = 1.0f / ySegments;
  float texDeltaX = 1.0f / xSegments;

  for (unsigned int y = 0; y < nVertsY; y++)
    for (unsigned int x = 0; x < nVertsX; x++) {
      Vertex vert;
      vert.Pos = float3(deltaX * x, 0.f, -deltaY * y);
      vert.Norm = float3(0, 0, 1.0f);
      vert.Tex = float2(texDeltaX * x, deltaY * y);
...
bool CTriMesh::CreateGLResources() {
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(float4) * m_Vertices.size(), pVertices,
               GL_DYNAMIC_DRAW);
  V_RETURN_OGL_ERROR();

  glGenBuffers(1, &m_glNormalBuffer);
  glBindBuffer(GL_ARRAY_BUFFER, m_glNormalBuffer);
  glBufferData(GL_ARRAY_BUFFER, sizeof(float4) * m_Vertices.size(), pNormals,
               GL_DYNAMIC_DRAW);
  V_RETURN_OGL_ERROR();

  glGenBuffers(1, &m_glTexCoordBuffer);
  glBindBuffer(GL_ARRAY_BUFFER, m_glTexCoordBuffer);
  glBufferData(GL_ARRAY_BUFFER, sizeof(float2) * m_Vertices.size(), pTexCoords,
               GL_DYNAMIC_DRAW);
  V_RETURN_OGL_ERROR();
  ...
  glBufferData(GL_ELEMENT_ARRAY_BUFFER,
               sizeof(unsigned int) * 3 * m_Faces.size(), pIndices,
               GL_STATIC_DRAW);
  V_RETURN_OGL_ERROR();
...
GLuint CTriMesh::GetVertexBuffer() const { return m_glVertexBuffer; }

GLuint CTriMesh::GetNormalBuffer() const { return m_glNormalBuffer; }
