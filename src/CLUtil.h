...
class CLUtil {
 public:
  //! Determines the OpenCL global work size given the number of data elements
  //! and threads per workgroup
  static size_t GetGlobalWorkSize(size_t DataElemCount, size_t LocalWorkSize);

  //! Builds a CL program
  static cl_program BuildCLProgramFromMemory(
      cl_device_id Device, cl_context Context, const std::string &SourceCode,
...
#define V_RETURN_FALSE_CL(expr, errmsg)                                       \
  do {                                                                        \
    cl_int e = (expr);                                                        \
    if (CL_SUCCESS != e) {                                                    \
      std::cerr << "Error: " << errmsg << " [" << CLUtil::GetCLErrorString(e) \
                << "]" << std::endl;                                          \
      return false;                                                           \
    }                                                                         \
  } while (0)
